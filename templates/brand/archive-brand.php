      ...
      <div class="col-md-3 brands-archive__sidebar" id="brands-filters">
        <div class="brands-filters">
          ...
          <div class="brands-filters__section">
            ...
            <!-- Динамические фильтры из таксономии brand_feature -->
            <?php
            ...
            ?>
              <div class="brands-filters__group">
                <h4><?php echo esc_html($filter->name); ?></h4>
                <?php foreach ($child_terms as $term) : ?>
                  <label class="brands-filters__checkbox">
                    <input type="checkbox" name="brand_filters[]" value="<?php echo esc_attr($term->term_id); ?>">
        ...
        <div class="brands-results">
          <div class="brands-results__header">
            <div class="brands-results__filters">
              <!-- Регион: показываем ТОЛЬКО родителей -->
              ...
              <!-- Город: dropdown SELECT вместо текстового поля -->
              <select id="brands-city-select" class="form-control" style="display:inline-block; width:48%; max-width:220px; margin-left:4%;">
...
<script>
// Заполняем города при выборе региона
document.getElementById('brands-region-select').addEventListener('change', function() {
  const regionId = this.value;
  const citySelect = document.getElementById('brands-city-select');
  citySelect.innerHTML = '<option value="">Населённый пункт</option>';

  if (regionId) {
    // AJAX запрос для получения городов (детей региона)
    wp.ajax.send('get_region_cities', {
